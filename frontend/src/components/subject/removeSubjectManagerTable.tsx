...
export function RemoveSubjectManagerTable({
  entity: subjectList,
  handleRemoveSubjects,
}: RemoveSubjectManagerTableProps) {
  ...
  useEffect(() => {
    if (debouncedQueryFilter) {
      setCurrentPage(1);
      const filteredSubject = subjectList.filter((subject) =>
        subject.name.includes(debouncedQueryFilter)
      );
