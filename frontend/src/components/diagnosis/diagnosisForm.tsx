...
type GenerateResponsesFormProps = {
  setStudentResponseList: React.Dispatch<React.SetStateAction<MockExamDiagnosisResponse[]>>
  selectPlaceholder: string
  dragAndDropPlaceholder: string
}

export function GenerateResponsesForm({ setStudentResponseList, selectPlaceholder, dragAndDropPlaceholder }: GenerateResponsesFormProps) {
  const [mockExamOptionsList, setMockExamOptionsList] = useState<SelectOptionProps[]>([]);
  ...
  const generateStudentsResponse = useMutation({
    mutationFn: async ({ mockExamSelected, studentRecordsExcelFile }: GenerateStudentsResponseForm) => {
      const formData = new FormData();
      formData.append('studentsMockExamsAnswers', studentRecordsExcelFile);
      
      const response = await fetch(`http://localhost:8080/mock-exam/${mockExamSelected.value}/responses`,
        {
          method: 'POST',
          body: formData
        })
        ...
        if (response.status === 200) {
          ...
          successAlert(`Respostas para o simulado ${mockExamSelected.label} salvas com sucesso!`);
        }
    ...
    function disableSubmitButton() {
      ...
      return formState.isSubmitting || !mockExamSelected.value || !studentRecordsExcelFile
    }

    return (
      <FormProvider {...formMethods}>
      <form onSubmit={handleSubmit(handleUploadFile)} encType='multipart/form-data' className="w-full space-y-6">
        <div className='flex flex-col gap-3 justify-around align-middle'>
          <div className='block w-52'>
            <span className='block mb-2 text-sm mt-4'>
              { selectPlaceholder }
            </span>
            <div>
              <InfiniteSelect
                ...
                handleSelect={(option: SelectOptionProps) => setValue('mockExamSelected', option)}
